...
#             connections[room].remove(str(websocket.client))
...
async def websocket_endpoint(websocket: WebSocket, host: str, client_id: str):
    ...
    try:
        while True:
            ...
            await manager.broadcast(data, room) # 주석?
    except WebSocketDisconnect:
        await manager.disconnect(websocket, room)
        await manager.broadcast(json.dumps({"type": "text", "content": f"{client_id} left the chat"}), room)

@app.get("/connections")
def get_connections():
    return {
        server_name: [str(websocket.client) for websocket in websockets]
        for server_name, websockets in manager.active_connections.items()
    }
    # return {
    #     server_name: [str(websocket.client) for websocket in websockets]
...
# async def upload_image(host: str, client_id: str, data: ImageData):
#     print('이미지전달!')
#     # 받은 이미지를 다른 클라이언트에게 브로드캐스트
#     for connection in connections[host]:
#         await connection.send_text(data.image)
@app.post("/stream/{host}/{client_id}")
async def upload_image(host: str, client_id: str, data: ImageData):
    print('이미지전달!')
    image_data = data.image  # base64 인코딩된 이미지 데이터
    json_data = json.dumps({"type": "image", "content": image_data})
    
    # 연결된 모든 클라이언트에게 이미지 전송
    for connection in connections[host]:
